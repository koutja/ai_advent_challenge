...
`план утверждён`/`валидация`) переживает перезапуск (SQLite) и попадает в
system-блок запроса, чтобы ассистент учитывал ограничения в рассуждении.

Команды CLI: `/approve` (утвердить план), `/accept` (принять после валидации),
`/next` (planning→execution после `/approve`, execution→validation), `/rework`,
`/pause`, `/resume`. В web-чате те же действия — кнопка «✔ План» и команды;
отказ показывается toast-сообщением с объяснением.

...
- **Web-фронтенд — обязательный этап, а не опция**: после любого этапа, добавляющего
  CLI-возможность, тот же функционал обязательно доводится до web-интерфейса (HTTP +
  страница), который вызывает то же ядро `Agent`. CLI и web всегда идут парой.

## Архитектура модуля `agent/` (гибрид)

Ядро агента — отдельный пакет `agent`, не зависящий от UI. Сверху два фронтенда,
оба вызывают одно и то же ядро: CLI (REPL-чат) и web (HTTP-сервер + простая
HTML-страница). **Оба фронтенда обязательны: web реализуется всегда после CLI**
(последовательность: сначала CLI, затем обязательно web на том же ядре).
...
│   └── web/main.go   # ФРОНТЕНД Web (обязателен, после CLI): HTTP-сервер + статика, вызывает то же ядро

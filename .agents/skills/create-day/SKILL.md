---
name: create-day
description: "Создание каркаса нового дня day_NN для AI Advent Challenge: Go-модуль с подключением общего пакета llm через replace, Makefile, README. Использовать при старте новой задачи дня, чтобы не дублировать соглашения репозитория."
---
...
вопросов при старте следующего дня. Демонстрацию работы дня записывает
пользователь сам — простой записью видео через среду разработки.
...
5. **Демо/видео вне задачи**: запись видео агент не создаёт и не запускает —
   демонстрацию пользователь записывает сам через среду разработки.
...
Демонстрацию работы дня пользователь записывает самостоятельно после
завершения дня — простой записью видео через среду разработки.
